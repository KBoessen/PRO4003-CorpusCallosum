...
par.node.elec.act(1).cond.value.vec(target_nodes) = ...
    par.node.elec.act(1).cond.value.vec(target_nodes) * 0.5;

% Myelin wraps 50%
par.myel.geo.numlamellae.value.vec(target_nodes) = par.myel.geo.numlamellae.value.ref * 0.5;
...
fprintf('RUNNING MODEL 50%% Myelin 50%% Density at nodes 5, 6, 7\n')
Model(par, fullfile(saveDirectory, 'CorpusCallosum50Mye50Na_nodes5to7.mat'));
...
title('Action Potentials at All Nodes (Myelin 50, Na 50, Nodes 5 to 7)');
grid on;
hold off;
